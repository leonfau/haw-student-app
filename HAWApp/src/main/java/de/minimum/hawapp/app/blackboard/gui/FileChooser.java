...
public class FileChooser extends ListActivity {
	...
	private void fill(final File f) {
		...
		final List<Item> fls = new ArrayList<Item>();
		try {
			for (final File ff : dirs) {
				final Date lastModDate = new Date(ff.lastModified());
				final DateFormat formater = DateFormat.getDateTimeInstance();
				final String date_modify = formater.format(lastModDate);
				if (ff.isDirectory()) {

					final File[] fbuf = ff.listFiles();
					int buf = 0;
					if (fbuf != null) {
						buf = fbuf.length;
					} else {
	...
	private void onFileClick(final Item o) {
