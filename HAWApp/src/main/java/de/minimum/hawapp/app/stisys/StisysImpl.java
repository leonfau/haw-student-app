...
public class StisysImpl implements Stisys {
	...
	public String login(final String name, final String password) {
		...
		try {
			...
			final String param = "username=" + URLEncoder.encode(name, "UTF-8")
	...
	public Map<Integer, List<Result>> parseReportPage(final String html) {
		...
		final Elements tableframe = doc
				.select("#ergebnisuebersicht .tableframe");
		final Elements tablecontent = doc
				.select("#ergebnisuebersicht .tablecontent");

		for (int i = 0; i < tableframe.size(); i++) {
			// Log.i("StisysImple",
			// "Semester: " + tableframe.get(i).select("tr td").text());
			final List<Result> retList = new ArrayList<Result>();
			final int semester = Integer
					.parseInt(tableframe
							.get(i)
							.select("tr td")
							.text()
			...
			for (int j = 1; j < tablecontent.get(i).select("tr table tr")
					.size(); j++) {
				// Log.i("StisysImple",
				// "Modul: "
				// + tablecontent.get(i).select("tr table tr")
				// .get(j).select("td").get(1).text());
				final String name = tablecontent.get(i).select("tr table tr")
						.get(j).select("td").get(1).text();
				...
				final String prof = tablecontent.get(i).select("tr table tr")
						.get(j).select("td").get(3).text();
				...
				final String date = tablecontent.get(i).select("tr table tr")
						.get(j).select("td").get(5).text();
				...
				final String result = tablecontent.get(i).select("tr table tr")
						.get(j).select("td").get(7).text();
				...
				while (k <= 15 && !note) {
					if (result.equals(String.valueOf(k))) {
						res = new ResultPoints(semester, name, prof, date,
